function [Xnmr]= XLaguerre(r,n,m,x) %Esta XL contiene el Anm que está en el 
                                    % draft de Ultimate Laguerre Dropbox, de modo que LaguerreG
                                    % y XLaguerreG son Laguerre y XLaguerre multiplicados por
                                    % el mismo término para hacerlos gaussianos
...
Xnmr=(-1)^(n+1)*gamma(n+m+1)/(n+(m+1)/2)^m/gamma(n+1)*gamma(n+m+1)/pi*(-a1+(-1)^n*(a2+a3));
end