function [h] = plotPropagatedRayZYPlane(RayObject,ray_index,scale1,scale2,color,linewidth)
...
% plotPropagatedRayZYPlane(RayObject,ray_index,scale1,scale2,color).
  ...
  for z_index = 1:Nz
    ...
    rayTemp.y(z_index) = RayObject(z_index).yCoordinate(ray_index);
          
  end

  % Plot
  h = plot(rayTemp.z/scale2,rayTemp.y/scale1,...
